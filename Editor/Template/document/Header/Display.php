 ...
 */
	$sql="select * from document_header where section_id=".$sectionId;
	$row = Database::selectFirst($sql);
	if ($row) {
		$level=$row['level'];
		$output.='<td style="'.$sectionStyle.'" class="sectionTDheader'.$level.'">';
		$style='';
		...
		$output.='<h'.$level.' style="'.$style.'" class="HeaderDisplay">'.convertText($row['text']).'</h'.$level.'>';
		$output.='</td>';
	}
?>