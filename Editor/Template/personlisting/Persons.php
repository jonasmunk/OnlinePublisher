...
'<form xmlns="uri:Form" action="UpdatePersons.php" method="post">'.
...
$selected = array();
$sql="select persongroup_id from personlisting_persongroup where page_id=".$id;
$result = Database::select($sql);
while ($row = Database::next($result)) {
	$selected[]=$row['persongroup_id'];
}
Database::free($result);

$sql="select object.* from object where object.type='persongroup' order by object.title";
$result = Database::select($sql);
while ($row = Database::next($result)) {
	$gui.=
	'<row>'.
