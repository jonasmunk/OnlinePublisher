...
$enabled = $row['personsenabled'];
$label = $row['personslabel'];
$default = $row['personsdefault'];
$hidden = $row['personshidden'];
...
'<tab title="Personer" style="Hilited"/>'.
'<tab title="Nyheder" link="News.php"/>'.
'<tab title="Produkter" link="Products.php"/>'.
'</tabgroup>'.
'<content padding="5" background="true">'.
'<form xmlns="uri:Form" action="UpdatePersons.php" method="post" name="Formula" focus="data">'.
...
'</radiogroup>'.
'<box title="Sprog:">'.
'<textfield badge="Tekst:" name="label" hint="Den tekst der vises for personer">'.
encodeXML($label).
'</textfield>'.
'</box>'.
