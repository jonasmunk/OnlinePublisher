...
class SearchController extends TemplateController {
    ...
    function build() {
		...
		$data.='<types>';
		if ($row['pagesenabled']) {
			$data.='<type label="'.encodeXML($row['pageslabel']).'" default="'.($row['pagesdefault'] ? 'true' : 'false').'" hidden="'.($row['pageshidden'] ? 'true' : 'false').'" unique="page"/>';
		}
		if ($row['imagesenabled']) {
			$data.='<type label="'.encodeXML($row['imageslabel']).'" default="'.($row['imagesdefault'] ? 'true' : 'false').'" hidden="'.($row['imageshidden'] ? 'true' : 'false').'" unique="image"/>';
		}
		if ($row['filesenabled']) {
			$data.='<type label="'.encodeXML($row['fileslabel']).'" default="'.($row['filesdefault'] ? 'true' : 'false').'" hidden="'.($row['fileshidden'] ? 'true' : 'false').'" unique="file"/>';
		}
		if ($row['personsenabled']) {
			$data.='<type label="'.encodeXML($row['personslabel']).'" default="'.($row['personsdefault'] ? 'true' : 'false').'" hidden="'.($row['personshidden'] ? 'true' : 'false').'" unique="person"/>';
		}
		if ($row['newsenabled']) {
			$data.='<type label="'.encodeXML($row['newslabel']).'" default="'.($row['newsdefault'] ? 'true' : 'false').'" hidden="'.($row['newshidden'] ? 'true' : 'false').'" unique="news"/>';
		}
		if ($row['productsenabled']) {
			$data.='<type label="'.encodeXML($row['productslabel']).'" default="'.($row['productsdefault'] ? 'true' : 'false').'" hidden="'.($row['productshidden'] ? 'true' : 'false').'" unique="product"/>';
		}
		$data.='</types>';
		$data.='<!--dynamic-->';
		$data.='</search>';
	...
	function dynamic(&$state) {
		if (requestGetExists('query')) {
			...
			$xml .= '<query>'.encodeXML($query).'</query>';
			$xml .= '<types>';
