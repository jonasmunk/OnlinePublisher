...
class TestSchemaService extends UnitTestCase {
    ...
    function testIt() {
		...
		$this->assertEqual($sql,'`variant`,`height`,`width`,`imagegroup_id`,`framed`,`show_title`');
		...
		$this->assertEqual($sql,"`variant`='block',`height`=100,`width`=0,`imagegroup_id`=78,`framed`=0,`show_title`=1");
    }
}
?>