...
'<titlebar title="Browser statistik" icon="Tool/Statistics"/>'.
...
function getStatistics($type){
		...
		$currentMonth = date("m");
		$currentDay = date("d");
		if($type=='hour'){return generateBrowserStatistics(subtractHoursFromCurrentDate(1,"Y-m-d-H-i-s"),date("Y-m-d-H-i-s"),"%Y-%m-%d-%H-%i-%s");} else
		if($type=='day'){return generateBrowserStatistics(subtractHoursFromCurrentDate(24,"Y-m-d-H"),date("Y-m-d-H"),"%Y-%m-%d-%H");} else
		if($type=='7day'){return generateBrowserStatistics(subtractHoursFromCurrentDate(168,"Y-m-d"),date("Y-m-d"),"%Y-%m-%d");} else 
		if($type=='14day'){return generateBrowserStatistics(subtractHoursFromCurrentDate(336,"Y-m-d"),date("Y-m-d"),"%Y-%m-%d");} else 
		if($type=='month'){return generateBrowserStatistics(subtractHoursFromCurrentDate(720,"Y-m-d"),date("Y-m-d"),"%Y-%m-%d");} else 
		if($type=='6month'){return generateBrowserStatistics(subtractHoursFromCurrentDate(4320,"Y-m-d"),date("Y-m-d"),"%Y-%m-%d");} else
		if($type=='12month'){return generateBrowserStatistics(subtractHoursFromCurrentDate(8640,"Y-m-d"),date("Y-m-d"),"%Y-%m-%d");} else
		if($type=='5year'){return generateBrowserStatistics(subtractHoursFromCurrentDate(43200,"Y-m-d"),date("Y-m-d"),"%Y-%m-%d");} else
		if($type=='total'){return generateBrowserStatistics(date("Y-m-d",0),date("Y-m-d"),"%Y-%m-%d");} 
		if($type=='advanced'){
			$startdate = trim(requestPostDateTime('startdate'));
			$enddate = trim(requestPostDateTime('enddate'));
...
function getAdvancedDateSettings($startdate,$enddate){
	...
	if(strlen($startdate) > 0){
		$output = generateBrowserStatistics(date("Y-m-d-H-i-s",$startdate),date("Y-m-d-H-i-s",$enddate),"%Y-%m-%d-%H-%i-%s");
	}
	return $output;
}
...
function generateBrowserStatistics($fromDate = "", $toDate="",$dateFormat="%Y-%m"){
		...
		$output =	'<headergroup>'.
					...
					'<header title="Bruger agent"/>'.
					'</headergroup>';
		$sql = 'SELECT count(distinct session) as uniquehits, count(distinct ip) as networks, count(*) as hits, agent FROM statistics ';
		
			if(strlen($fromDate)>0)
				$sql.= $sqlFromTo;
			else
				$sql.= $sqlDefault;
		$sql.=' group by agent ORDER BY uniquehits DESC';
		$result = Database::select($sql);	
		$count = 0;
