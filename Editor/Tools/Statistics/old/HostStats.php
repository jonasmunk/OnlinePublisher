...
'<titlebar title="Server statistik" icon="Tool/Statistics"/>'.
...
function getStatistics($type){
		...
		$currentMonth = date("m");
		$currentDay = date("d");
		if($type=='hour'){return generateHostStatistics(subtractHoursFromCurrentDate(1,"Y-m-d-H-i-s"),date("Y-m-d-H-i-s"),"%Y-%m-%d-%H-%i-%s");} else
		if($type=='day'){return generateHostStatistics(subtractHoursFromCurrentDate(24,"Y-m-d-H"),date("Y-m-d-H"),"%Y-%m-%d-%H");} else
		if($type=='7day'){return generateHostStatistics(subtractHoursFromCurrentDate(168,"Y-m-d"),date("Y-m-d"),"%Y-%m-%d");} else 
		if($type=='14day'){return generateHostStatistics(subtractHoursFromCurrentDate(336,"Y-m-d"),date("Y-m-d"),"%Y-%m-%d");} else 
		if($type=='month'){return generateHostStatistics(subtractHoursFromCurrentDate(720,"Y-m-d"),date("Y-m-d"),"%Y-%m-%d");} else 
		if($type=='6month'){return generateHostStatistics(subtractHoursFromCurrentDate(4320,"Y-m-d"),date("Y-m-d"),"%Y-%m-%d");} else
		if($type=='12month'){return generateHostStatistics(subtractHoursFromCurrentDate(8640,"Y-m-d"),date("Y-m-d"),"%Y-%m-%d");} else
		if($type=='5year'){return generateHostStatistics(subtractHoursFromCurrentDate(43200,"Y-m-d"),date("Y-m-d"),"%Y-%m-%d");} else
		if($type=='total'){return generateHostStatistics(date("Y-m-d",0),date("Y-m-d"),"%Y-%m-%d");} 
		if($type=='advanced'){
			$startdate = trim(requestPostDateTime('startdate'));
			$enddate = trim(requestPostDateTime('enddate'));
...
function getAdvancedDateSettings($startdate,$enddate){
	...
	if(strlen($startdate) > 0){
		$output = generateHostStatistics(date("Y-m-d-H-i-s",$startdate),date("Y-m-d-H-i-s",$enddate),"%Y-%m-%d-%H-%i-%s");
	}
	return $output;
}
...
function generateHostStatistics($fromDate = "", $toDate="",$dateFormat="%Y-%m"){
		...
		$output =	'<headergroup>'.
					...
					'<header title="Bruger serverer"/>'.
					'</headergroup>';
		$sql = 'SELECT count(distinct session) as uniquehits, count(distinct ip) as networks, count(*) as hits, host FROM statistics ';
		
			if(strlen($fromDate)>0)
				$sql.= $sqlFromTo;
			else
				$sql.= $sqlDefault;
		$sql.=' group by host ORDER BY uniquehits DESC';
		$result = Database::select($sql);	
		$count = 0;
