...
'<titlebar title="Reference statistik" icon="Tool/Statistics"/>'.
...
function getStatistics($type){
		...
		$currentMonth = date("m");
		$currentDay = date("d");
		if($type=='hour'){return generateReferStatistics(subtractHoursFromCurrentDate(1,"Y-m-d-H-i-s"),date("Y-m-d-H-i-s"),"%Y-%m-%d-%H-%i-%s");} else
		if($type=='day'){return generateReferStatistics(subtractHoursFromCurrentDate(24,"Y-m-d-H"),date("Y-m-d-H"),"%Y-%m-%d-%H");} else
		if($type=='7day'){return generateReferStatistics(subtractHoursFromCurrentDate(168,"Y-m-d"),date("Y-m-d"),"%Y-%m-%d");} else 
		if($type=='14day'){return generateReferStatistics(subtractHoursFromCurrentDate(336,"Y-m-d"),date("Y-m-d"),"%Y-%m-%d");} else 
		if($type=='month'){return generateReferStatistics(subtractHoursFromCurrentDate(720,"Y-m-d"),date("Y-m-d"),"%Y-%m-%d");} else 
		if($type=='6month'){return generateReferStatistics(subtractHoursFromCurrentDate(4320,"Y-m-d"),date("Y-m-d"),"%Y-%m-%d");} else
		if($type=='12month'){return generateReferStatistics(subtractHoursFromCurrentDate(8640,"Y-m-d"),date("Y-m-d"),"%Y-%m-%d");} else
		if($type=='5year'){return generateReferStatistics(subtractHoursFromCurrentDate(43200,"Y-m-d"),date("Y-m-d"),"%Y-%m-%d");} else
		if($type=='total'){return generateReferStatistics(date("Y-m-d",0),date("Y-m-d"),"%Y-%m-%d");} 
		if($type=='advanced'){
			$startdate = trim(requestPostDateTime('startdate'));
			$enddate = trim(requestPostDateTime('enddate'));
...
function getAdvancedDateSettings($startdate,$enddate){
	...
	if(strlen($startdate) > 0){
		$output = generateReferStatistics(date("Y-m-d-H-i-s",$startdate),date("Y-m-d-H-i-s",$enddate),"%Y-%m-%d-%H-%i-%s");
	}
	return $output;
}
...
function generateReferStatistics($fromDate = "", $toDate="",$dateFormat="%Y-%m"){
		$month = getMonth($id);
		...
		/*$sql = 'SELECT count(distinct session, referer) as uniquehits, count(distinct ip) as networks, count(*) as hits FROM statistics';
		...
		*/
		$output =	'<headergroup>'.
					...
					'<header title="Reference adresse"/>'.
					'</headergroup>';
		$sql = 'SELECT count(distinct session) as uniquehits, count(distinct ip) as networks, count(*) as hits, referer FROM statistics ';
		...
		$sql.= ' group by referer order by uniquehits DESC';
		
		$result = Database::select($sql);	
		$count = 0;
		/*echo "Maxhits:".$maxHits."<br>";
		echo $sql;*/
