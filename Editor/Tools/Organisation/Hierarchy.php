...
while ($row = Database::next($result)) {
	$gui.='<element icon="Role/'.($row['sex']==1 ? 'Male' : 'Female').'" title="'.encodeXML($row['title']).'" link="PersonProperties.php?id='.$row['id'].'&amp;group=0" target="Right"/>';
}
...
$result = Database::select($sql); 
while ($row = Database::next($result)) {
	$gui.='<element icon="Element/Folder" title="'.encodeXML($row['title']).'" link="Persongroup.php?id='.$row['id'].'" target="Right">';
	...
	while ($person = Database::next($result_person)) {
		$gui.='<element icon="Role/'.($person['sex']==1 ? 'Male' : 'Female').'" title="'.concatenatePersonName(encodeXML($person['firstname']),encodeXML($person['middlename']),encodeXML($person['surname'])).'" link="PersonProperties.php?id='.$person['object_id'].'&amp;group='.$row['id'].'" target="Right"/>';
	}
	$gui.='</element>';
...
$result = Database::select($sql); 
while ($row = Database::next($result)) {
		$gui.='<element icon="Role/Administrator" title="'.encodeXML($row['title']).'" link="RoleProperties.php?id='.$row['id'].'" target="Right"/>';
}
Database::free($result);
$gui.=
