...
class CacheService {
	...
	static function createPageCache($id,$path,$html) {
		...
		Database::delete($sql);
		$sql = "insert into page_cache (page_id,path,html,stamp,version) values (".Database::int($id).",".Database::text($_SERVER['REQUEST_URI']).",".$html.",now(),".Database::int(SystemInfo::getDate()).")";
		Database::insert($sql);
	}
