hui.ui.listen({
	...
	_renderRecord : function(data) {
		...
		for (var i=0; i < data.length; i++) {
			...
			var status = hui.build('td',{parent:row});
			hui.log(data[i])
			if (data[i].status===1) {
				hui.dom.setText(status,'OK');
			} else if (data[i].status===-1) {
				hui.dom.setText(status,'Afvist');
			} else if (data[i].status===0) {
				var a = hui.build('a',{parent:status,text:'Slet','class':'delete',data:data[i].id,href:'javascript://'});
				hui.listen(a,'click',this._clickDelete.bind(this));
			}
		};
	},
	_clickDelete : function(e) {
		e = hui.event(e);
		var a = e.element;
		hui.ui.confirmOverlay({element:e.element,text:'Er du sikker?',okText:'Ja, slet',cancelText:'Nej',$ok:function() {
			var id = a.getAttribute('data');
			this._deleteUsage(id);
		}.bind(this)})
	},
	_deleteUsage : function(id) {
		hui.ui.request({
			url : op.context+'services/waterusage/delete_usage.php',
			parameters : {id:id},
			message : {start:'Sletter registrering...',success:'Registreringen er slettet'},
			$success : function() {
				this._fetch(this.number);
			}.bind(this),
			$failure : function() {
				hui.ui.showMessage({text:'Det lykkedes desværre ikke at slette registreringen',icon:'common/warning',duration:3000});
			}
		})
	},
