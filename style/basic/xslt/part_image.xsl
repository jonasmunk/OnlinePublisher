...
<xsl:attribute name="href"><xsl:value-of select="$path"/>util/images/?id=<xsl:value-of select="img:link/@image"/></xsl:attribute>
</xsl:when>
</xsl:choose>
</xsl:template>
...
<xsl:value-of select="$path"/><xsl:text>util/images/?id=</xsl:text>
...
<xsl:text>&amp;maxwidth=</xsl:text><xsl:value-of select="img:transform/@max-width"/>
</xsl:if>
<xsl:if test="img:transform/@max-height and $highquality='false'">
<xsl:text>&amp;maxheight=</xsl:text><xsl:value-of select="img:transform/@max-height"/>
</xsl:if>
