...
<html>
    ...
    <head>
        ...
        <xsl:call-template name="util:style-inline">
			<xsl:with-param name="compiled">body{padding:0;margin:0;font-family:'Georgia',Times,'Times New Roman',serif}body.font{font-family:'Lora','Georgia',Times,'Times New Roman',serif}form{margin:0}img{border:none}a{word-break:break-word}@media screen and (max-width:700px){div.document_column{display:block}.document_column{width:auto!important;padding:0!important}.part_image_image{max-width:100%;height:auto}}@media screen and (min-width:700px){.document_row{display:table;width:100%}.document_row_body{display:table-row}div.document_column{display:table-cell;vertical-align:top}}</xsl:with-param>
        </xsl:call-template>
